...
const ratingRadio = ratingForm.querySelectorAll('input[type="radio"]');

// event listeners
ratingForm.addEventListener('submit', ratingFormSubmit);
ratingRadio.forEach((radio) => {
	radio.addEventListener('change', () => closeError());
});

/**
 * @name ratingFormSubmit
 *@description used to handel form submition
 * @param {SubmitEvent} e
 */
function ratingFormSubmit(e) {
	e.preventDefault();
	const submitButton = e.currentTarget.querySelector('button[type="submit"]');
	const checkedRadio = getRatingChecked(e.currentTarget);
	...
	thanksSectionCreation(container, checkedRadio);
}
 ...
 */
function getRatingChecked(form) {
	const ratingChekedButton = [
		...form.querySelectorAll('input[type="radio"]'),
	].filter((button) => button.checked);
	// if no  checked
	if (ratingChekedButton.length === 0) {
		return null;
	}
	// if checked
	return ratingChekedButton[0];
}

function openError() {
	document.querySelector('.erorr').classList.add('open');
}

function closeError() {
	document.querySelector('.erorr').classList.remove('open');
}

function thanksSectionCreation(container, checkedRadio) {
	container.innerHTML = `<section class="thanks">
...
<p class="rating-info">
    You selected
    <span class="rating-selected">${checkedRadio.value}</span> out of 5
</p>
